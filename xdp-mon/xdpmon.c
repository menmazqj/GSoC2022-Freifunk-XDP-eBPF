...
const char *object_filenames = "xdpmon_xdp.o";
...
void user_xdp_detach_single(struct , struct xdp_config *cfg) {
...
static struct stats_record* stats_collect(int bpf_map_fd, __u32 map_type, struct stats_record *stats_rec)
{
...
void stats_poll(int bpf_map_fd, __u32 map_type, int interval)
{
	struct stats_record prev, record = {0};

	enum print_mode {ADDR, PROTO, PORT, DEFAULT};

	stats_collect(bpf_map_fd, map_type, &record);
